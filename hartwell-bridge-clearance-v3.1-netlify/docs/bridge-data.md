...
Map provider settings live in `data/settings.json`, with live overrides coming from the Netlify function `/.netlify/functions/app-settings`. To use Google Maps, add a restricted browser key in Netlify as `GOOGLE_MAPS_API_KEY`. Optional map styling can use `GOOGLE_MAPS_MAP_ID`. Keep the static JSON key blank because browser map keys are visible to visitors. If the key is blank or Google Maps fails to load, the app automatically uses the backup map provider so clearance checks, GPS sorting, bridge cards, and markers still work.
